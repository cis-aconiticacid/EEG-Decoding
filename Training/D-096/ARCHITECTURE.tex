...
\begin{tikzpicture}[node distance=9mm and 10mm]
  \node[font=\sffamily\bfseries\Large,anchor=west] at (-1.7,6.2)
    {D-096: Position-Aware Waveform/Frequency Cross-Attention Classifier};
  ...
  \node[note,above=2mm of add] {$z_0=z_s+P$ (position added once)};
  ...
  \node[note,anchor=west] at (-1.7,-10.1)
    {Blue: waveform and cross-attentive signal path \quad Orange: frequency path \quad Green: position context \quad Purple: temporal--spatial trunk \quad Gray: readout};
\end{tikzpicture}
