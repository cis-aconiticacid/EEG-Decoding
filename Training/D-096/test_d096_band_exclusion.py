import torch
from d096_model import D096CrossAttentionClassifier, frequency_bin_centres


def test_notch_removes_target_preserves_other_dft_components():
    torch.set_num_threads(2)
    model = D096CrossAttentionClassifier(torch.ones(62, 3), excluded_band_hz=(18, 28)).eval()
...
def test_default_is_identity_and_initialization_is_matched():
    ...
    base = D096CrossAttentionClassifier(torch.ones(62, 3))
    torch.manual_seed(17)
    excluded = D096CrossAttentionClassifier(torch.ones(62, 3), excluded_band_hz=(18, 28))
