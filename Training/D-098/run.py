"""Train one independent D098 model for each EEG-ImageNet participant."""
...
from eegdecoding.subject_data import split_evidence  # noqa: E402
...
EXPECTED_CLASSES = 80
EXPECTED_PARTICIPANTS = tuple(range(16))
...
def participant_archive(participant: int) -> str:
    """Map the sixteen participant IDs to the two official source archives."""
    if participant not in EXPECTED_PARTICIPANTS:
        raise ValueError(f"participant must be one of {EXPECTED_PARTICIPANTS}")
    return "EEG-ImageNet_1.pth" if participant < 8 else "EEG-ImageNet_2.pth"


def load_participant_data(cfg: Config) -> dict[str, Any]:
    """Load one participant while preserving documented source incompleteness."""
    participant = int(cfg["subject"])
    crop_start, crop_end = map(int, cfg["crop"])
    archive = load_part(cfg["archive"])
    chosen = [
        (source_index, row)
        for source_index, row in enumerate(archive["dataset"])
        if int(row["subject"]) == participant
    ]
    if not chosen:
        raise AssertionError(f"participant {participant} is absent from {cfg['archive']}")

    label_lookup = {label: index for index, label in enumerate(archive["labels"])}
    rows = len(chosen)
    waveform = torch.empty((rows, CHANNELS, crop_end - crop_start), dtype=torch.float32)
    labels = np.empty(rows, dtype=np.int64)
    images = np.empty(rows, dtype="U128")
    trial_ids = np.empty(rows, dtype="U64")
    source_indices = np.empty(rows, dtype=np.int64)
    source_split = np.empty(rows, dtype="U5")
    positions = {label: 0 for label in range(EXPECTED_CLASSES)}
    for local, (source_index, row) in enumerate(chosen):
        label = label_lookup[row["label"]]
        position = positions[label]
        positions[label] += 1
        waveform[local] = row["eeg_data"][:, crop_start:crop_end].float()
        labels[local] = label
        images[local] = str(row["image"])
        trial_ids[local] = f"subject-{participant:02d}-{source_index:06d}"
    ...
    present = counts > 0
    if tuple(waveform.shape[1:]) != (CHANNELS, crop_end - crop_start):
        raise AssertionError("unexpected participant waveform shape")
    if not np.all(counts[present] == 50) or int(present.sum()) < 78:
        raise AssertionError(f"unexpected participant class counts: {counts.tolist()}")
    ...
    return {
        ...
        "present_classes": np.flatnonzero(present).astype(int).tolist(),
    }


def prepare_data(cfg: Config, model: D098CrossAttentionClassifier) -> PreparedData:
    """Load participant data and fit input normalization on its training split only."""
    data = load_participant_data(cfg)
...
def build_model(
    ...
    """Construct the D098 classifier from the experiment configuration."""
    return D098CrossAttentionClassifier(
        coordinates,
        classes=cfg["classes"],
        ...
        add_position_after_cross_attention=cfg.get("add_position_after_cross_attention", True),
    ).to(device)


def build_optimizer(
    model: D098CrossAttentionClassifier, cfg: Config
...
def train_epoch(
    model: D098CrossAttentionClassifier,
...
def evaluate_test_once(
    ...
    metrics = {
        ...
        "macro_class_accuracy": float(per_class[present_classes].mean()),
        "macro_all_80_output_classes_accuracy": float(per_class.mean()),
        "present_test_classes": int(present_classes.sum()),
    ...
    for candidate in cfg["microbatch_candidates"]:
        ...
        try:
            ...
            if loss is not None:
                del loss
            torch.cuda.empty_cache()
    raise RuntimeError(f"no D098 microbatch fits: {failures}")
...
def architecture_record(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "frequency_branch": {
            ...
            "mask_below_hz": cfg.get("frequency_mask_below_hz"),
        },
        "cross_attention_fusion": {
            ...
            "post_cross_attention_position_added": cfg.get("add_position_after_cross_attention", True),
        },
        "embeddings": {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--participant", type=int, required=True, help="participant ID from 0 to 15")
...
def load_config(path: Path, participant: int) -> Config:
    """Load the shared D098 configuration and resolve one participant run."""
    cfg = json.loads(path.read_text(encoding="utf-8"))
    if participant not in EXPECTED_PARTICIPANTS or participant not in cfg["participants"]:
        raise AssertionError("D098 participant must be listed in the 0--15 contract")
    if cfg["crop"] != EXPECTED_CROP or cfg["classes"] != EXPECTED_CLASSES:
        raise AssertionError("D098 crop/class contract differs")
    if cfg["label_smoothing"] != 0.0:
        raise AssertionError("D098 requires ordinary cross entropy with label_smoothing=0")
    if cfg.get("frequency_branch") is not True:
        raise AssertionError("D098 requires the frequency branch for cross-attention")
    if cfg.get("frequency_mask_below_hz") != 25.0:
        raise AssertionError("D098 fixes the frequency mask below 25 Hz")
    if cfg.get("add_position_after_cross_attention") is not True:
        raise AssertionError("D098 retains post-cross-attention position")
    cfg["subject"] = participant
    cfg["archive"] = participant_archive(participant)
    cfg["task"] = f"D098-participant-{participant:02d}-masked-below25-with-position"
    cfg["run_dir"] = f"outputs/participants/participant_{participant:02d}/runs"
    cfg["report_dir"] = f"outputs/participants/participant_{participant:02d}/reports"
    cfg["train_split"] = f"participant {participant}; first 30 source-order images per available class"
    cfg["test_split"] = f"participant {participant}; remaining 20 source-order images per available class"
    return cfg


def configure_cuda(cfg: Config) -> torch.device:
    """Validate the optional GPU lock and enable the D098 CUDA settings."""
    configured_gpu = cfg.get("gpu_uuid")
    visible_gpu = os.environ.get("CUDA_VISIBLE_DEVICES")
    if (configured_gpu is not None and visible_gpu != configured_gpu) or not torch.cuda.is_available():
        raise RuntimeError("D098 must use the configured local GPU UUID when one is provided")
...
def validate_split(prepared: PreparedData) -> dict[str, Any]:
    """Verify first-30/last-20 counts for every class present in the archive."""
    split = split_evidence(prepared["data"])
    class_counts = prepared["data"]["class_counts"]
    expected_train = [30 if count else 0 for count in class_counts]
    expected_test = [20 if count else 0 for count in class_counts]
    if split["per_class"]["official_train"] != expected_train:
        raise AssertionError("D098 per-class training split differs")
    if split["per_class"]["test"] != expected_test:
        raise AssertionError("D098 per-class test split differs")
    if split["official_train_test_image_overlap"] != 0:
        raise AssertionError("D098 train/test image IDs overlap")
    split["class_counts"] = class_counts
    split["present_classes"] = prepared["data"]["present_classes"]
    split["missing_classes"] = [
        label for label, count in enumerate(class_counts) if count == 0
    ]
    return split
...
def train_model(
    ...
    if curve_path.exists():
        raise FileExistsError(f"refusing to append to existing D098 curve: {curve_path}")
