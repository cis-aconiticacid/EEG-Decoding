...
def test_multiscale_frontend_and_full_architecture_shapes():
    model = D098CrossAttentionClassifier(coordinates(), dropout=0.0, head_dropout=0.0)
...
def test_coordinate_normalization_and_block_contract():
    ...
    model = D098CrossAttentionClassifier(normalized, dropout=0.0, head_dropout=0.0)
...
def test_wrong_input_shape_is_rejected():
    model = D098CrossAttentionClassifier(coordinates())
    try:
