...
\begin{tikzpicture}[node distance=9mm and 10mm]
  \node[font=\sffamily\bfseries\Large,anchor=west] at (-1.7,6.2)
    {D-098: One Independent Cross-Attention Model per Participant};

  % Shared input and preprocessing.
  \node[input] (raw) at (0,2.4)
    {Participant EEG crop\\40--440 ms at 1 kHz\\$x\in\mathbb{R}^{B\times62\times400}$};
  ...
  \node[freq,right=of fft] (fproj)
    {Fixed mask of bins below 25 Hz\\(15.625, 19.53125, 23.4375)\\Linear $17\!\to\!192$};
  ...
  \node[note,above=2mm of add] {Fixed D-097 winner: $z_0=z_s+P$};
  ...
  \node[note,below=7mm of loss]
    {16 independent fits: first 30 images per available class\\Remaining 20 per class tested once; no cross-participant sharing};
