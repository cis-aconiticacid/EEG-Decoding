"""Train D097 factorial groups on subject 0 and evaluate each endpoint once."""
...
def prepare_data(cfg: Config, model: D097CrossAttentionClassifier) -> PreparedData:
...
def build_model(
    ...
    """Construct the D097 classifier from the experiment configuration."""
    return D097CrossAttentionClassifier(
        coordinates,
        classes=cfg["classes"],
...
def build_optimizer(
    model: D097CrossAttentionClassifier, cfg: Config
...
def train_epoch(
    model: D097CrossAttentionClassifier,
    ...
    for candidate in cfg["microbatch_candidates"]:
        ...
        try:
            ...
            if loss is not None:
                del loss
            torch.cuda.empty_cache()
    raise RuntimeError(f"no D097 microbatch fits: {failures}")
...
def load_config(path: Path) -> Config:
    """Load the JSON configuration and validate the fixed D097 contract."""
    ...
    if (
        ...
        raise AssertionError("D097 subject/crop/class contract differs")
    if cfg["label_smoothing"] != 0.0:
        raise AssertionError("D097 requires ordinary cross entropy with label_smoothing=0")
    if cfg.get("frequency_branch") is not True:
        raise AssertionError("D097 requires the frequency branch for cross-attention")
    return cfg


def configure_cuda(cfg: Config) -> torch.device:
    """Validate the optional GPU lock and enable the D097 CUDA settings."""
    ...
    if (configured_gpu is not None and visible_gpu != configured_gpu) or not torch.cuda.is_available():
        raise RuntimeError("D097 must use the configured local GPU UUID when one is provided")
...
def validate_split(prepared: PreparedData) -> dict[str, Any]:
    ...
    if split["official_train"] != EXPECTED_TRAIN_SAMPLES or split["test"] != EXPECTED_TEST_SAMPLES:
        raise AssertionError("D097 first30/last20 split count differs")
    if (
        split["per_class"]["official_train"] != [30] * EXPECTED_CLASSES
        or split["per_class"]["test"] != [20] * EXPECTED_CLASSES
    ):
        raise AssertionError("D097 per-class first30/last20 split differs")
    if split["official_train_test_image_overlap"] != 0:
        raise AssertionError("D097 train/test image IDs overlap")
    return split
...
def train_model(
    ...
    if curve_path.exists():
        raise FileExistsError(f"refusing to append to existing D097 curve: {curve_path}")
