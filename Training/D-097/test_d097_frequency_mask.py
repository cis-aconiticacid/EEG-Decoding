import torch
from d097_model import D097CrossAttentionClassifier, frequency_bin_centres


def test_notch_removes_target_preserves_other_dft_components():
    torch.set_num_threads(2)
    model = D097CrossAttentionClassifier(torch.ones(62, 3), excluded_band_hz=(18, 28)).eval()
...
def test_default_is_identity_and_initialization_is_matched():
    ...
    base = D097CrossAttentionClassifier(torch.ones(62, 3))
    torch.manual_seed(17)
    excluded = D097CrossAttentionClassifier(torch.ones(62, 3), excluded_band_hz=(18, 28))
...
def test_factorial_frequency_mask_and_post_cross_position_switch():
    ...
    masked = D097CrossAttentionClassifier(
        torch.ones(62, 3),
        dropout=0.0,
        head_dropout=0.0,
    ...
    torch.manual_seed(17)
    with_position = D097CrossAttentionClassifier(
        torch.ones(62, 3), dropout=0.0, head_dropout=0.0,
        add_position_after_cross_attention=True,
    ).eval()
    without_position = D097CrossAttentionClassifier(
        torch.ones(62, 3), dropout=0.0, head_dropout=0.0,
        add_position_after_cross_attention=False,
    ).eval()
