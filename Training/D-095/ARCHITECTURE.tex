...
\begin{tikzpicture}[node distance=9mm and 10mm]
  \node[font=\sffamily\bfseries\Large,anchor=west] at (-1.7,6.2)
    {D-095: Hybrid Multi-Scale Waveform/Frequency Temporal--Spatial Classifier};
  ...
  \node[freq,right=of fft] (fproj)
    {Optional band-bin mask\\for exclusion ablation\\Linear $17\!\to\!192$};
  ...
  % Frequency residual gate.
  \node[wave,text width=42mm,right=15mm of zw,yshift=-18mm] (gate)
    {Scalar residual gate per token\\$g=\sigma(W[\mathrm{LN}(z_w);\mathrm{LN}(z_f)]+b)$\\$z_s=z_w+g\,z_f$};
  \node[note,above=2mm of gate] {gate shape $[B,62,16,1]$; initialized to $g=0.1$};
  \draw[flow] (zw) -- (gate);
  \draw[flow] (zf) -- (gate);
  ...
  \node[sum] (add) at (5.2,-4.8) {$+$};
  \node[note,above=2mm of add]
    {$z_0=z_s+e_{\mathrm{coord}}+e_{\mathrm{electrode}}+e_{\mathrm{time}}$};
  \draw[guide] (coord.east) -| (add.north);
  \draw[guide] (eid.east) -- (add.west);
  \draw[guide] (tid.east) -| (add.south);
  \draw[flow] (gate.south) |- (add.east);
  ...
  \node[note,anchor=west] at (-1.7,-10.1)
    {Blue: waveform and gated signal path \quad Orange: frequency path \quad Green: learned/coordinate embeddings \quad Purple: temporal--spatial trunk \quad Gray: readout};
\end{tikzpicture}
